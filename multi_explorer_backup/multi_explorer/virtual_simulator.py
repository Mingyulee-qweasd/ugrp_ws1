...
# 시작 위치를 가깝게 설정 (초기 merge를 위해)
ROBOT_STARTS = [(0.0, 0.0), (0.5, 0.0), (0.25, 0.4)]

class VirtualRobot:
    def __init__(self, x, y, robot_id):
        self.x=x; self.y=y; self.theta=0.0; self.id=robot_id
        self.vx=0.0; self.vtheta=0.0
...
class VirtualSimulator(Node):
    def __init__(self):
        ...
        self._sim_time  = 0.0  # 누적 시뮬레이션 시간 (초)

        # /clock 퍼블리셔 - use_sim_time=true 환경에서 필수
        ...
        self.get_logger().info(
            f'Virtual Simulator 시작 — {NUM_ROBOTS}대')
    ...
    def _tf(self, robot):
        ...
        # odom → base_footprint  (SLAM이 map→odom을 담당)
        t2 = TransformStamped()
        t2.header.stamp    = now
